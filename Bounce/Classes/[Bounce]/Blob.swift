...
class Blob
{
    ...
    func update() {
        ...
        if isViewMode {
            ...
            if grabSelectionTouch === nil && grabSelectionGesture == false {
    ...
    func drawMarkers() {
        ...
        if isViewMode {
            ...
            for i in 0..<tri.count {
                ...
                let x1 = meshNodes.data[t.i1].animX
                let y1 = meshNodes.data[t.i1].animY
                
                let x2 = meshNodes.data[t.i2].animX
                let y2 = meshNodes.data[t.i2].animY
                
                let x3 = meshNodes.data[t.i3].animX
                let y3 = meshNodes.data[t.i3].animY
        ...
        var dir = Math.angleToVector(twistRotation)
        var len = 50.0 * inflateScale
        var start = CGPoint(x: center.x + dir.x * len, y: center.y + dir.y * len)
        var end = CGPoint(x: center.x - dir.x * len, y: center.y - dir.y * len)
        
        ShaderProgramMesh.shared.colorSet(r: 0.0, g: 1.0, b: 1.0, a: 0.8)
        ShaderProgramMesh.shared.lineDraw(p1: start, p2: end, thickness: 2.0)
        */
        ...
        for nodeIndex in 0..<meshNodes.count {
            ...
            ShaderProgramMesh.shared.rectDraw(x: GLfloat( node.animX - 1.0 ), y: GLfloat( node.animY - 1.0 ), width: GLfloat( 3.0 ), height: GLfloat( 3.0 ))
