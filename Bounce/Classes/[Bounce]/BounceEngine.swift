...
class BounceEngine {
    ...
    var loadName: String? {
        if _loadName != nil {
            if _loadName!.characters.count > 0 {
                return _loadName!
            }
        }
        return nil
    ...
    var animationPower: CGFloat = 0.0
    ...
    var grabSelectionStartAnimationGuideOffset = CGPoint.zero
    ...
    var viewMode:ViewMode = .grab {
        didSet {
            ...
            BounceEngine.postNotification(BounceNotification.viewModeChanged)
        }
    }
    ...
    func pinchBegin(pos:CGPoint, scale:CGFloat) {
        ...
        if sceneMode == .view && viewMode == .grab {
    ...
    func updateSecondaryViewGrabGesture() {
        if let blob = grabSelectionBlob {
            if grabSelectionPanning {
                let guideX = grabSelectionStartAnimationGuideOffset.x + (panPos.x - grabSelectionPanStart.x)
                let guideY = grabSelectionStartAnimationGuideOffset.y + (panPos.y - grabSelectionPanStart.y)
                blob.animationGuideOffset = CGPoint(x: guideX, y: guideY)
            }
            if grabSelectionPinching {
                blob.inflateScale = grabSelectionStartInflateScale * pinchScale
                if blob.inflateScale > 2.0 { blob.inflateScale = 2.0 }
                if blob.inflateScale < 0.25 { blob.inflateScale = 0.25 }
            }
            if grabSelectionRotating {
                blob.twistRotation = grabSelectionStartTwistRotation + rotation
                if blob.twistRotation > Math.PI_2 { blob.twistRotation = Math.PI_2 }
                if blob.twistRotation < -Math.PI_2 { blob.twistRotation = -Math.PI_2 }
            }
        }
    }
    ...
    func attemptSecondaryViewGrabGesture(withPoint point: CGPoint) {
        ...
        if let touchBlob = selectBlobAtPoint(point) {
            ...
            if multipleBlobsSelected == false {
                ...
                print("SUCCESS SecondaryViewGrabGesture -> Locking Values")
                
                
                grabSelectionBlob = touchBlob
                grabSelectionStartInflateScale = touchBlob.inflateScale
                grabSelectionStartTwistRotation = touchBlob.twistRotation
                grabSelectionStartAnimationGuideOffset = touchBlob.animationGuideOffset
                ...
                print("FAIL SecondaryViewGrabGesture -> NO BLOB")
            }
        } else {
            print("FAIL SecondaryViewGrabGesture -> NO BLOB")
