...
class BounceViewController : GLViewController, UIGestureRecognizerDelegate//, URLSessionDelegate
{
    ...
    func setUpNew(image:UIImage, sceneRect:CGRect, portraitOrientation:Bool) {
        ...
        let thumbBB = CGSize(width: 256.0, height: 256.0)
        let thumbSize = thumbBB.getAspectFit(image.size).size
        let thumb = image.resize(thumbSize)
        
        //CGSize.getAspectFit(<#T##CGSize#>)
        ...
        _ = FileUtils.saveImagePNG(image: thumb, filePath: ApplicationController.shared.recentImageThumbPath)
    ...
    func showSaveDialog() {
        ...
        let root = ApplicationController.shared.root
        guard root.popoverPresentationController == nil else {
            print("YOU SHALL NOT PASS!")
            return
        }
        
        if let saveVC = root.storyboard.instantiateViewController(withIdentifier: "save_dialog") as? SaveDialogViewController {
            
            root.showPopover(withVC: saveVC)
            
            
            if let loadName = engine.loadName {
                //Save Over, Or New Save?
    ...
    func saveScene(filePath:String?) {
        saveScene(filePath: filePath, scene: engine.scene, isRecent: false)
    }
    ...
    func saveRecentScene() {
        ...
        saveScene(filePath: "recent_scene.json", scene:scene, isRecent: true)
    }
    
    func saveScene(filePath: String?, scene: BounceScene, isRecent: Bool) {
        if let path = filePath , path.characters.count > 0 {
            var info = [String:AnyObject]()
            
            var recent = isRecent
            if path == "recent_scene.json" { recent = true }
            
            if recent {
                scene.isRecent = true
                scene.imageName = ApplicationController.shared.recentImageName
                scene.imagePath = ApplicationController.shared.recentImagePath
                scene.thumbPath = ApplicationController.shared.recentImageThumbPath
            }
            ...
            do {
                ...
                if fileData != nil {
                    ...
                    if isRecent == false {
                        ...
                        var isSaveOver: Bool = ApplicationController.shared.savedFiles.fileExists(filePath: path)
                        
                        if isSaveOver == false {
                            
                            scene.imageName = Config.shared.uniqueString
                            scene.imagePath = String(scene.imageName) + ".png"
                            scene.thumbPath = String(scene.imageName) + "_thumb.png"
                            
                        } else {
                            
                            ApplicationController.shared.deleteScene(filePath: path)
                            
                            scene.imagePath = String(scene.imageName) + ".png"
                            scene.thumbPath = String(scene.imageName) + "_thumb.png"
                        }
                        
                        FileUtils.copyFile(from: ApplicationController.shared.recentImageThumbPath, to: FileUtils.getDocsPath(filePath: scene.thumbPath))
                        FileUtils.copyFile(from: ApplicationController.shared.recentImagePath, to: FileUtils.getDocsPath(filePath: scene.imagePath))
                        
                        
                    } else {
                        //recentImageName
                        //recentImagePath
                        //recentImageThumbPath
                    }
