...
class ApplicationController
{
    ...
    internal var _savedFiles: SavedFileList?
    var savedFiles: SavedFileList {
        if _savedFiles == nil {
            _savedFiles = SavedFileList()
            _savedFiles!.load()
        }
        return _savedFiles!
    }
    ...
    var recentImageName: String {
        return "recent_x1234x_save"
    }
    
    var recentImagePath: String {
        return FileUtils.getDocsPath(filePath: "\(recentImageName).png")
    }
    
    var recentImageThumbPath: String {
        return FileUtils.getDocsPath(filePath: "\(recentImageName)_thumb.png")
    }
    ...
    func deleteScene(filePath: String?) -> Void {
        ...
        if let fileData = FileUtils.loadData(filePath) {
            ...
            if let info = parsedInfo {
                ...
                if let sceneInfo = info["scene"] as? [String:AnyObject] {
                    scene.load(info: sceneInfo)
                    FileUtils.deleteFile(scene.imagePath)
                    FileUtils.deleteFile(scene.imagePath)
                }
            }
        }
