...
class Math {
    ...
    static let epsilon:CGFloat = 0.015
    
    class func angleToVector(_ radians: CGFloat) -> CGPoint {
        return CGPoint(x: sin(radians), y: -cos(radians))
    }
    
    class func angleToVectord(_ degrees: CGFloat) -> CGPoint {
        return CGPoint(x: sind(degrees), y: -cosd(degrees))
    }
